...
from dagerputy.settings_utils import environ_get_list, force_bool
from dagerputy.dagerputy_config import *
...
if os.environ.get('DAGERPUTY_FROM_EMAIL'):
    DEFAULT_FROM_EMAIL = os.environ['DAGERPUTY_FROM_EMAIL']
...
TEMPLATES = [{
    ...
    'OPTIONS': {
        'context_processors': [
            ...
            'dagerputy.context_processors.global_settings',
        ],
...
DAGERPUTY_PLUGINS_ENABLED_PARSED = []
for plugin in DAGERPUTY_PLUGINS_ENABLED.split(","):
    # Hack to clean up if versions of plugins specified
    exploded = re.split(r'[<>=]+', plugin)
    DAGERPUTY_PLUGINS_ENABLED_PARSED.append(exploded[0])

INSTALLED_APPS += tuple(DAGERPUTY_PLUGINS_ENABLED_PARSED)
