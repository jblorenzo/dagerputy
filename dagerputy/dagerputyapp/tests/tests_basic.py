...
import os
import requests
from dagerputy.dagerputyapp.graphite import parse_metric
from dagerputy.dagerputyapp.alert import update_alert_plugins, AlertPlugin
from dagerputy.dagerputyapp.models import (
...
from dagerputy.dagerputyapp.calendar import get_events
from dagerputy.dagerputyapp.views import StatusCheckReportForm
from dagerputy.dagerputyapp import tasks
...
class TestCheckRun(LocalTestCase):
    ...
    @patch('dagerputy.dagerputyapp.alert.AlertPlugin._send_alert')
    @patch('dagerputy.dagerputyapp.alert.AlertPlugin._send_alert_update')
    ...
    @patch('dagerputy.dagerputyapp.graphite.requests.get', fake_graphite_response)
    ...
    @patch('dagerputy.dagerputyapp.graphite.requests.get')
    ...
    @patch('dagerputy.dagerputyapp.graphite.requests.get', fake_graphite_series_response)
    ...
    @patch('dagerputy.dagerputyapp.graphite.requests.get', fake_empty_graphite_response)
    ...
    @patch('dagerputy.dagerputyapp.graphite.requests.get', fake_slow_graphite_response)
    ...
    @patch('dagerputy.dagerputyapp.models.jenkins_check_plugin.get_job_status')
    ...
    @patch('dagerputy.dagerputyapp.models.jenkins_check_plugin.get_job_status', throws_timeout)
    ...
    @patch('dagerputy.dagerputyapp.models.requests.get', fake_http_200_response)
    ...
    @patch('dagerputy.dagerputyapp.models.requests.get', throws_timeout)
    ...
    @patch('dagerputy.dagerputyapp.models.requests.get', fake_http_404_response)
...
class TestDutyRota(LocalTestCase):
    ...
    @patch('dagerputy.dagerputyapp.models.requests.get', fake_gcal_response)
    def test_duty_rota(self):
        events = get_events()
        self.assertEqual(events[0]['summary'], 'troels')

    @patch('dagerputy.dagerputyapp.models.requests.get', fake_recurring_response)
    ...
    @patch('dagerputy.dagerputyapp.models.requests.get', fake_recurring_response_notz)
    ...
    @patch('dagerputy.dagerputyapp.models.requests.get',
...
class TestWebInterface(LocalTestCase):
    ...
    def test_about_page(self):
        response = self.client.get(reverse('about-dagerputy'))
            ...
            'httpstatuscheck': [
                {
                    ...
                    'frequency': 5,
                    'debounce': 0,
                    'endpoint': u'http://remedee.com/post_tests',
                    'username': None,
                    'password': None,
                    'text_match': u'text',
                    'status_code': u'201',
...
class TestCleanUpTask(LocalTestCase):
    ...
    def test_cleanup_single_batch(self):
        with patch('dagerputy.dagerputyapp.tasks.clean_db.apply_async'):
            initial_results = StatusCheckResult.objects.all().count()
    ...
    @patch('dagerputy.dagerputyapp.tasks.clean_db.apply_async')
