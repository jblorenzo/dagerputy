...
import jenkins
from dagerputy.dagerputyapp import jenkins as dagerputy_jenkins
from dagerputy.dagerputyapp.models import JenkinsConfig
from dagerputy.dagerputyapp.models.jenkins_check_plugin import JenkinsStatusCheck
...
class TestGetStatus(unittest.TestCase):
    ...
    @patch("dagerputy.dagerputyapp.jenkins._get_jenkins_client")
    def test_job_passing(self, mock_jenkins):
        mock_jenkins.return_value = self.mock_client

        status = dagerputy_jenkins.get_job_status(self.mock_config, 'foo')
    ...
    def test_job_unknown(self, mock_jenkins):
        ...
        status = dagerputy_jenkins.get_job_status(self.mock_config, 'unknown-job')
    ...
    def test_job_no_build(self, mock_jenkins):
        ...
        with self.assertRaises(Exception):
            dagerputy_jenkins.get_job_status(self.mock_config, 'job-unbuilt')
    ...
    def test_job_no_good_build(self, mock_jenkins):
        ...
        mock_jenkins.return_value = self.mock_client
        status = dagerputy_jenkins.get_job_status(self.mock_config, 'job-no-good-build')
        expected = {
            'active': True,
            'succeeded': False,
            'job_number': 1,
