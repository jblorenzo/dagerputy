...
from dagerputy.dagerputyapp import alert
from dagerputy.dagerputyapp.utils import dagerputy_needs_setup
...
def subscriptions(request):
    ...
    return render(request, 'dagerputyapp/subscriptions.html', {
        'services': services,
        'users': users,
    ...
    class Meta:
        ...
        widgets.update({
            ...
            'password': forms.PasswordInput(attrs={
                'style': 'width: 30%',
                # Prevent auto-fill with saved Dagerputy log-in credentials:
...
class CheckCreateView(LoginRequiredMixin, CommonCreateView):
    template_name = 'dagerputyapp/statuscheck_form.html'
...
class StatusCheckDeleteView(LoginRequiredMixin, CommonDeleteView):
    ...
    template_name = 'dagerputyapp/statuscheck_confirm_delete.html'


class StatusCheckDetailView(LoginRequiredMixin, CommonDetailView):
    model = StatusCheck
    context_object_name = 'check'
    template_name = 'dagerputyapp/statuscheck_detail.html'
...
class UserProfileUpdateAlert(LoginRequiredMixin, View):
    template = loader.get_template('dagerputyapp/alertpluginuserdata_form.html')
    model = AlertPluginUserData
...
class PluginSettingsView(LoginRequiredMixin, View):
    template = loader.get_template('dagerputyapp/plugin_settings_form.html')
    model = AlertPlugin
...
class ServicePublicListView(TemplateView):
    model = Service
    context_object_name = 'services'
    template_name = "dagerputyapp/service_public_list.html"
...
class ServiceDeleteView(LoginRequiredMixin, CommonDeleteView):
    ...
    context_object_name = 'service'
    template_name = 'dagerputyapp/service_confirm_delete.html'
...
class InstanceDeleteView(LoginRequiredMixin, CommonDeleteView):
    ...
    context_object_name = 'instance'
    template_name = 'dagerputyapp/instance_confirm_delete.html'
...
class StatusCheckReportView(LoginRequiredMixin, TemplateView):
    template_name = 'dagerputyapp/statuscheck_report.html'
...
class SetupView(View):
    template = loader.get_template('dagerputyapp/setup.html')

    def get(self, request):
        if not dagerputy_needs_setup():
...
def about(request):
    ...
    from dagerputy import version

    return render(request, 'dagerputyapp/about.html', {
        'dagerputy_version': version,
    })
